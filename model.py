...
class ST_GCN(nn.Module):
    ...
    def forward(self, g, inputs):
        if self.data_type == 'Polar': # 如果输入的是极坐标，则把角度与距离整合在一起
        #     h1 = inputs['angle'].float()
        #     h2 = inputs['distance'].float()
            h = inputs['polar'].float()
        ...
        if self.data_type == 'Cartesian':
            ...
            h = inputs['location'].float()
            
        # 每层加入resnet结构
        h = F.relu(self.GraphConv1(g, h))
        x = h
        ...
        with g.local_scope():
            ...
            # h1 = inputs['location1'].float()
            # h2 = inputs['location2'].float()
            # h1 = F.relu(self.GraphConv_1_1(g, h1))
            # x = h1
            # h1 = F.relu(self.GraphConv_1_2(g, h1)) + x
            # x = h1
